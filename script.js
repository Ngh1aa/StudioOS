...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelector("#notificationButton").addEventListener("click", () => { closeHelp(); const popover = document.querySelector("#notificationPopover"); popover.hidden = !popover.hidden; document.querySelector("#notificationButton").setAttribute("aria-expanded", String(!popover.hidden)); });
  document.querySelector("#helpButton")?.addEventListener("click", () => { closeNotifications(); const popover = document.querySelector("#helpPopover"); const button = document.querySelector("#helpButton"); popover.hidden = !popover.hidden; button.setAttribute("aria-expanded", String(!popover.hidden)); });
  document.querySelector("#helpDocsButton")?.addEventListener("click", () => { closeHelp(); toast("Help center is coming soon."); });
  document.addEventListener("click", (event) => { if (!event.target.closest(".topbar-actions")) { closeNotifications(); closeHelp(); } });
  ...
  document.querySelectorAll("[data-action]").forEach((button) => button.addEventListener("click", () => { const action = button.dataset.action; if (["workspace","focus-options","activity-options","activity","comment","profile-menu"].includes(action)) toast(action === "comment" ? "Comment composer is coming soon." : action === "profile-menu" ? "Account menu is coming soon." : "This workspace action is coming soon."); }));
